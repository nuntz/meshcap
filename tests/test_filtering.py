...
from meshcap.filter import FilterParser, FilterEvaluator, FilterError, parse_filter, evaluate_filter
...
class TestFilterParser:
    ...
    def test_simple_or_expression(self):
        ...
        assert result == [('port', 'portnum', 'text'), ('port', 'portnum', 'position'), 'or']
    ...
    def test_complex_expression_with_parentheses(self):
        ...
        parser = FilterParser()
        result = parser.parse(['src', 'node', 'A', 'and', '(', 'port', 'text', 'or', 'port', 'position', ')'])
        expected = [
            ('node', 'src', 'A'),
            ('port', 'portnum', 'text'),
    ...
    def test_invalid_is_syntax(self):
        ...
        parser = FilterParser()
        with pytest.raises(FilterError, match="'is' must be followed by 'encrypted' or 'plaintext'"):
    ...
    def test_user_filter_parsing(self):
        ...
        with pytest.raises(FilterError, match="'src' must be followed by 'node' or 'user'"):
...
class TestFilterEvaluator:
    ...
    def test_encryption_filter(self):
        ...
        rpn: List[Union[Tuple[str, str, str], str]] = [('encryption', 'status', 'plaintext')]
        assert evaluator.evaluate_rpn(rpn, packet) is True
        
        rpn: List[Union[Tuple[str, str, str], str]] = [('encryption', 'status', 'encrypted')]
        assert evaluator.evaluate_rpn(rpn, packet) is False
    ...
    def test_and_operator(self):
        """Test AND operator."""
        evaluator = FilterEvaluator()
        packet = self.create_packet(fromId='nodeA', decoded={'portnum': 'TEXT_MESSAGE_APP'})
        
        # Both conditions true
        rpn: List[Union[Tuple[str, str, str], str]] = [('node', 'src', 'nodeA'), ('port', 'portnum', 'text'), 'and']
        assert evaluator.evaluate_rpn(rpn, packet) is True
        
        # First true, second false
        rpn: List[Union[Tuple[str, str, str], str]] = [('node', 'src', 'nodeA'), ('port', 'portnum', 'position'), 'and']
        assert evaluator.evaluate_rpn(rpn, packet) is False
        
        # First false, second true
        rpn: List[Union[Tuple[str, str, str], str]] = [('node', 'src', 'nodeB'), ('port', 'portnum', 'text'), 'and']
        assert evaluator.evaluate_rpn(rpn, packet) is False
        
        # Both false
        rpn: List[Union[Tuple[str, str, str], str]] = [('node', 'src', 'nodeB'), ('port', 'portnum', 'position'), 'and']
        assert evaluator.evaluate_rpn(rpn, packet) is False
    ...
    def test_or_operator(self):
        ...
        # Both conditions true
        rpn: List[Union[Tuple[str, str, str], str]] = [('node', 'src', 'nodeA'), ('port', 'portnum', 'text'), 'or']
        assert evaluator.evaluate_rpn(rpn, packet) is True
        
        # First true, second false
        rpn: List[Union[Tuple[str, str, str], str]] = [('node', 'src', 'nodeA'), ('port', 'portnum', 'position'), 'or']
        assert evaluator.evaluate_rpn(rpn, packet) is True
        
        # First false, second true
        rpn: List[Union[Tuple[str, str, str], str]] = [('node', 'src', 'nodeB'), ('port', 'portnum', 'text'), 'or']
        assert evaluator.evaluate_rpn(rpn, packet) is True
        
        # Both false
        rpn: List[Union[Tuple[str, str, str], str]] = [('node', 'src', 'nodeB'), ('port', 'portnum', 'position'), 'or']
        assert evaluator.evaluate_rpn(rpn, packet) is False
    ...
    def test_invalid_expression_result(self):
        ...
        # Too many operands
        rpn: List[Union[Tuple[str, str, str], str]] = [('node', 'src', 'nodeA'), ('port', 'portnum', 'text')]
...
class TestIntegrationScenarios:
    ...
    def test_complex_parentheses_expression(self):
        ...
        expression = ['(', 'src', 'node', 'A', 'or', 'dst', 'node', 'A', ')', 'and', 
                     '(', 'port', 'text', 'or', 'port', 'position', ')']
        rpn = parse_filter(expression)
