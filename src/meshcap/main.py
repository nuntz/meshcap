...
class MeshCap:
    ...
    def run(self):
        ...
        if self.args.read_file:
            ...
            self._read_packets_from_file(self.args.read_file, self.args.no_resolve, self.args.verbose)
        ...
        if self.args.write_file:
            try:
                ...
                print(f"Error: Could not open write file '{self.args.write_file}': {e}", file=sys.stderr)
                sys.exit(1)
        ...
        def packet_handler(packet, interface):
            self._on_packet_received(packet, interface, self.args.no_resolve, self.args.verbose)
        ...
        count_msg = f" (stopping after {self.target_count} packets)" if self.target_count else ""
    ...
    def _format_packet(self, packet, interface, no_resolve, verbose=False):
        ...
        """
        timestamp = datetime.fromtimestamp(packet.get('rxTime', 0), timezone.utc).strftime('%Y-%m-%d %H:%M:%S')
        ...
        raw_from_id = packet.get('fromId', 'unknown')
        raw_to_id = packet.get('toId', 'unknown')

        if no_resolve or interface is None:
            from_id = raw_from_id
            to_id = raw_to_id
        else:
            from_id = raw_from_id
            ...
            if interface and hasattr(interface, 'nodes'):
                if raw_from_id in interface.nodes:
                    node = interface.nodes[raw_from_id]
                    if 'user' in node and 'longName' in node['user']:
                        from_id = node['user']['longName']

                if raw_to_id in interface.nodes:
                    node = interface.nodes[raw_to_id]
                    if 'user' in node and 'longName' in node['user']:
                        to_id = node['user']['longName']
        ...
        return f"[{timestamp}] Ch:{channel_hash} {signal} {from_id}→{to_id} {packet_type}: {payload}"
...
def main():
    ...
    parser.add_argument(
        "filter",
        nargs="*",
        help="Filter expression (e.g., 'src node A and port text')"
    )
